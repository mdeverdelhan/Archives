...
int hydra_get_options(char *options[]) {
	...
	if ( !strncmp(tmp, "http-", 5) || !strncmp(tmp, "https-", 6)) {
		options[i++] = "-m";
	...
	} else if ( !strcmp(tmp, "ldap2")) {
		options[i++] = "-m";
		widget = lookup_widget(GTK_WIDGET(wndMain), "entLDAPDN");
		options[i++] = (char *) gtk_entry_get_text((GtkEntry *) widget);

	} else if ( !strcmp(tmp, "ldap3")) {
		options[i++] = "-m";
		widget = lookup_widget(GTK_WIDGET(wndMain), "entLDAPDN");
		options[i++] = (char *) gtk_entry_get_text((GtkEntry *) widget);

	} else if ( !strcmp(tmp, "smbnt")) {
		memset(smbntparm, 0, 4);
		...
		j = gtk_spin_button_get_value_as_int((GtkSpinButton *) widget);
		snprintf(sapr3id, 10, "%d", j);
		options[i++] = "-m";
		options[i++] = sapr3id;		

	} else if (!strcmp(tmp, "http-proxy")) {
...
void
on_wndMain_destroy                     (GtkObject       *object,
                                        gpointer         user_data)
{
	if (hydra_pid != 0){
		kill(hydra_pid, SIGTERM);	
...
gboolean
on_entTargetFile_button_press_event    (GtkWidget       *widget,
...
gboolean
on_entUsernameFile_button_press_event  (GtkWidget       *widget,
                                        GdkEventButton  *event,
                                        gpointer         user_data)
{
...
gboolean
on_entPassFile_button_press_event      (GtkWidget       *widget,
                                        GdkEventButton  *event,
                                        gpointer         user_data)
{
...
gboolean
on_entColonFile_button_press_event      (GtkWidget       *widget,
                                        GdkEventButton  *event,
                                        gpointer         user_data)
{
	hydra_select_file((GtkEntry *) widget, "Select colon seperated user,password list");
	gtk_widget_grab_focus(widget);
	return TRUE;
}
