...
void
icq_header(char *buf, unsigned short cmd, unsigned long uin)
{
  buf[0] = 0x02;
  buf[1] = 0x00;
  buf[2] = cmd & 0xff;
...
int
icq_login(int s, char *login, char *pass)
{
...
int
icq_disconnect(int s, char *login)
{
  unsigned long uin = strtoul(login, NULL, 10);
  char buf[64];
...
int
start_icq(int sock, unsigned long int ip, int port, FILE * output, char *miscptr, FILE * fp)
{
...
void
service_icq(unsigned long int ip, int sp, unsigned char options, char *miscptr, FILE * fp, int port)
{
