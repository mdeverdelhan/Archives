...
int
start_cvs(int s, unsigned long int ip, int port, unsigned char options,
...
*/
  ...
  if (hydra_data_ready_timed(s, 5, 0) > 0) {
    ...
    if (strstr(buffer, "I LOVE YOU\n")) {
      hydra_report_found_host(port, ip, "cvs", fp);
      hydra_completed_pair_found();
      if (memcmp(hydra_get_next_pair(), &HYDRA_EXIT, sizeof(HYDRA_EXIT)) == 0) {
	return 3;
      }
    } else if ( strstr(buffer, "no such user")) {
...
void
service_cvs(unsigned long int ip, int sp, unsigned char options,
